import resKDI from "../images/res-KDI.jpg";
import resKJY from "../images/res-KJY.jpg";
import resKJYong from "../images/res-KJYeong.jpg";
import resPSC from "../images/res-PSC.jpg";
import resYCH from "../images/res-YCH.jpg";
import resJHH from "../images/res-JHH.jpg";
import resHMH from "../images/res-HMH.jpg";
import resLYR from "../images/res-LYR.jpg";

import resProjCYS from "../images/res-project-CYS.png";
import resProjKKI from "../images/res-project-KKI.jpg";
import resProjLS from "../images/res-project-LS.png";
import resProjPH from "../images/res-project-PH.png";
import resProjPHM from "../images/res-project-PHM.jpg";
import resProjPJ from "../images/res-project-PJ.png";
import resProjSSB from "../images/res-project-SSB.jpg";

function getGlobalRes() {
  return [
    {
      name: "염춘호",
      image: resYCH,
      details: [
        "서울시립대학교 국제도시과학대학원",
        "글로벌건설학과 주임교수",
        "서울시 동대문구 서울시립대로 163, 법학관 517호",
        "University of Seoul",
        "International School of Urban Sciences",
        "Professor",
        "Law School 517, Seoulsiripdaero 163, Dongdaemun-gu, Seoul 02504 KOREA",
        "전    화: 02-6490-5154",
        "연락처: 010-2969-2504",
        "Email : chunhoy7@uos.ac.kr",
      ],
    },
    {
      name: "고주연",
      image: resKJY,
      details: [
        "서울시립대학교 도시과학연구원 박사/연구교수",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "전    화: 02-6490-5320",
        "이메일: gojy@uos.ac.kr",
      ],
    },
    {
      name: "김대일",
      image: resKDI,
      details: [
        "공학박사/연구교수",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일:  kkim019@uos.ac.kr",
      ],
    },
    {
      name: "이예린",
      image: resLYR,
      details: [
        "지리학박사/박사후연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일:  sophiekelee@uos.ac.kr",
      ],
    },
    {
      name: "박성찬 Sungchan Park",
      image: resPSC,
      details: [
        "선임연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: psc1006@uos.ac.kr",
      ],
    },
    {
      name: "김지영",
      image: resKJYong,
      details: [
        "연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: jiyoung8@uos.ac.kr",
      ],
    },
    {
      name: "김정현",
      image: undefined,
      details: [
        "연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: powergee101@gmail.com",
      ],
    },
    {
      name: "이가람",
      image: undefined,
      details: [
        "연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: river01@uos.ac.kr",
      ],
    },
    {
      name: "오규민",
      image: undefined,
      details: [
        "연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: darkblue31415@gmail.com",
      ],
    },
    {
      name: "정현호",
      image: resJHH,
      details: [
        "연구원",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일: jhh001202@gmail.com",
      ],
    },
  ];
}

function getProjectRes() {
  return [
    {
      name: "염춘호",
      image: resYCH,
      details: [
        "연구책임자",
        "연구 총괄 및 스마트교통 기술 연구",
        "서울시립대학교 국제도시과학대학원",
        "글로벌건설학과 주임교수",
        "서울시 동대문구 서울시립대로 163, 법학관 517호",
        "전    화: 02-6490-5154",
        "연락처: 010-2969-2504",
        "Email : chunhoy7@uos.ac.kr",
      ],
    },
    {
      name: "한만희",
      image: resHMH,
      details: [
        "대외협력부총장",
        "지역정책, 주택정책, 부동산금융, 해외건설",
        "서울시립대학교 도시과학대학원 교수",
        "서울시 동대문구 서울시립대로 163, 법학관 512호",
        "전    화: 02-6490-5145",
        "Email : mhhandc@uos.ac.kr",
      ],
    },
    {
      name: "최윤수",
      image: resProjCYS,
      details: [
        "공동연구원 (일반공동)",
        "측지 및 측량 분야 수출 기술 발굴",
        "서울시립대학교 공간정보학과",
        "서울시 동대문구 서울시립대로 163, 21세기관 509호",
        "전    화: 02-6490-2887",
        "Email : choiys@uos.ac.kr",
      ],
    },
    {
      name: "이신",
      image: resProjLS,
      details: [
        "공동연구원 (일반공동)",
        "스마트도시 관련 다학제 연계 구상",
        "서울시립대학교 국제도시과학대학원",
        "서울시 동대문구 서울시립대로 163, 법학관 514호",
        "전    화: 02-6490-5151",
        "Email : shinlee714@uos.ac.kr",
      ],
    },
    {
      name: "박준",
      image: resProjPJ,
      details: [
        "공동연구원 (일반공동)",
        "국외 스마트도시 도시 계획 분야 수요 조사 연구",
        "서울시립대학교 국제도시과학대학원",
        "서울시 동대문구 서울시립대로 163, 법학관 516호",
        "전    화: 02-6490-5153",
        "Email : joon.park@uos.ac.kr",
      ],
    },
    {
      name: "김광일",
      image: resProjKKI,
      details: [
        "공동연구원 (일반공동)",
        "인적 네트워크 구축 방안 연구",
        "서울시립대학교 중국어문화학과",
        "서울시 동대문구 서울시립대로 163, 인문학관 5212호",
        "전    화: 02-6490-2594",
        "Email : kikim@uos.ac.kr",
      ],
    },
    {
      name: "서승범",
      image: resProjSSB,
      details: [
        "공동연구원 (일반공동)",
        "국외 스마트환경 기술 수요 조사 연구",
        "서울시립대학교 국제도시과학대학원",
        "서울시 동대문구 서울시립대로 163, 법학관 510호",
        "전    화: 02-6490-5135",
        "Email : sbseo7@uos.ac.kr",
      ],
    },
    {
      name: "박현",
      image: resProjPH,
      details: [
        "공동연구원 (일반공동)",
        "대학원 내 과제 신설 및 특강, 세미나 개최 계획 추진",
        "서울시립대학교 국제도시과학대학원",
        "서울시 동대문구 서울시립대로 163, 법학관 513호",
        "전    화: 02-6490-5146",
        "Email : hpark@uos.ac.kr",
      ],
    },
    {
      name: "박혜민",
      image: resProjPHM,
      details: [
        "공동연구원 (일반공동)",
        "환경 분야 수출 기술 및 규제 발굴",
        "서울시립대학교 국제도시과학대학원",
        "서울시 동대문구 서울시립대로 163, 법학관 515호",
        "전    화: 02-528-5152",
        "Email : minpark@uos.ac.kr",
      ],
    },
    {
      name: "고주연",
      image: resKJY,
      details: [
        "공동연구원 (전임연구인력)",
        "국내외 스마트도시 정책 및 현황 분석",
        "서울시립대학교 도시과학연구원 박사/연구교수",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "전    화: 02-6490-5320",
        "이메일: gojy@uos.ac.kr",
      ],
    },
    {
      name: "김대일",
      image: resKDI,
      details: [
        "공동연구원 (전임연구인력)",
        "스마트도시 기술 현황 조사 분석/스마트도시수출 거점HUB  플랫폼 개발",
        "공학박사/연구교수",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일:  kkim019@uos.ac.kr",
      ],
    },
    {
      name: "이예린",
      image: resLYR,
      details: [
        "공동연구원 (전임연구인력)",
        "스마트도시수출 거점국가 선정(지역학)",
        "지리학박사",
        "서울시립대학교 국제도시 및 인프라연구센터[도시과학연구원]",
        "서울특별시 동대문구 서울시립대로 163, 21세기관 104-1호",
        "이메일:  sophiekelee@uos.ac.kr",
      ],
    },
  ];
}

export { getGlobalRes, getProjectRes };
