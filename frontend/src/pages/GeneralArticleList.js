...
function GeneralArticleList(props) {
  ...
  useEffect(() => {
    ...
    getArticles(page, perPage, kind)
      ...
      .catch((err) => {
        alert(
          "게시글을 가져올 수 없습니다.. 잠시 뒤에 다시 시도하십시오: " +
            err?.response?.status
        );
        history.push("/");
      });
    ...
    <React.Fragment>
      {articles === undefined ? (
        ...
        <React.Fragment>
          ...
          <p className="list-total-info">
            {total} 건의 게시물이 검색되었습니다.
          </p>
                  ...
                  <tr className="list-row">
                    ...
                    <td className="list-title">
                      <Tooltip title={element.title}>
                        <h3 onClick={() => handleArticleClick(element)}>
                          {element.title}
                        </h3>
                      </Tooltip>
                    </td>
          ...
          <div className="list-pagination">
            <Pagination
              count={
                Math.floor(total / perPage) + (total % perPage > 0 ? 1 : 0)
              }
              page={page}
