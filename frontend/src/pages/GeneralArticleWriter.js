...
function GeneralArticleWriter(props) {
  ...
  useEffect(() => {
    ...
    if (articleId) {
      getArticle(articleId)
        ...
        .catch((ex) => {
          alert(
            "글을 가져올 수 없었습니다. 정상적인 경로로 접근했는지 확인해주세요."
          );
          history.push(link);
        });
    }
  ...
  async function saveArticle() {
    ...
    try {
      ...
      alert(
        "파일 또는 게시글을 업로드하는데 실패하였습니다. 다시 시도해주세요."
      );
  ...
  return (
    <React.Fragment>
      <Prompt
        when={routeToMove === undefined}
        message="아직 작성중인 게시글을 저장하지 않았습니다. 정말 나가시겠습니까?"
      ></Prompt>
      ...
      {selectedFiles.length > 0 ? (
        <strong className="writer-caption">새로 업로드될 파일:</strong>
      ) : undefined}
      ...
      {uploadedFiles.length > 0 ? (
        <strong className="writer-caption">과거에 업로드한 파일:</strong>
      ) : undefined}
        ...
        {Object.keys(uploadedFilesInfo).length > 0
          ? uploadedFiles.map((element) => (
              <div>
                ...
                <IconButton
                  size="small"
                  onClick={getUploadedFileRemover(element)}
                >
                  <BackspaceIcon fontSize="inherit"></BackspaceIcon>
                </IconButton>
              </div>
